...
api_url_file_stop_write_all = (
    f"{config_api_file.get('endpoint')}/api/stop_write_folder_all"
)
#############################################################################################################################################
...
api_url_mongo_stop_write_all = (
    f"{config_api_mongodb.get('endpoint')}/api/stop_write_loop_all"
)
##############################################################################################################################################
...
api_url_di_dir_stop_loop = (
    f"{config_api_ni.get('endpoint')}/api/stop_read_di_dir_loop_to_redis"
)
#################################################################################################################################################
...
api_url_di_vit_stop_loop = (
    f"{config_api_ni.get('endpoint')}/api/stop_read_di_vit_loop_to_redis"
)
#################################################################################################################################################
...
api_url_di_syn_stop_loop = (
    f"{config_api_ni.get('endpoint')}/api/stop_read_di_syn_loop_to_redis"
)
#################################################################################################################################################
            ...
            return jsonify(
                {
                    ...
                    "values": f"Echec de la requêtte : {response.status_code} - {response.text}",
                }
            )

    except requests.exceptions.RequestException as e:
        return jsonify(
            {"status": response.status_code, "values": f"Exception level : {e}"}
        )
    ...
    try:
        ...
        if "host" not in data or "port" not in data:
            return (
                jsonify(
                    {
                        "error": f"Les données doivent contenir l'adresse ip du serveur et le port."
                    }
                ),
                400,
            )
        ...
        BACKGROUND_UDP = threading.Thread(
            target=collect_data_from_udp, args=(STOP_EVENT_COLLECT_UDP, udp)
        )
...
def api_read():
    ...
    try:
        ...
        BACKGROUND = threading.Thread(
            target=collect_data, args=(STOP_EVENT_COLLECT, udp, redis_conn, config)
        )
        # Démarrez le thread en arrière-plan
        BACKGROUND.start()
...
def udp_communication():
    try:
        ...
        udp_handler = UDPHandler(
            target_ip=data.get("host"), target_port=int(data.get("port"))
        )
        ...
        if action == "send":
            ...
            if message:
                ...
                return (
                    jsonify(
                        {
                            "error": "Le message doit être spécifié dans le corps de la requête JSON."
                        }
                    ),
                    400,
                )
